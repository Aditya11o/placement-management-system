...
import { 
  ...
  Plus, Globe, 
  ShieldCheck, Users,
  Trophy, X, Loader2, LayoutDashboard, GraduationCap, UserCircle
} from 'lucide-react';
...
const AlumniPortal: React.FC = () => {
    ...
    const [selectedJobForApplicants, setSelectedJobForApplicants] = useState<{id: string, title: string} | null>(null);
    ...
    return (
        <div className="max-w-7xl mx-auto p-6 space-y-8 animate-in fade-in duration-700">
            ...
            <div className="flex flex-wrap items-center gap-3 bg-gray-100 p-1.5 rounded-2xl w-fit mx-auto lg:mx-0">
                {[
                    { id: 'dashboard', name: 'Dashboard', icon: LayoutDashboard },
                    { id: 'mentorship', name: 'Mentorship', icon: GraduationCap },
                    { id: 'referrals', name: 'Referrals', icon: Briefcase },
                    { id: 'profile', name: 'Professional Profile', icon: UserCircle },
                ].map((tab) => {
                    const Icon = tab.icon;
            ...
            {activeTab === 'dashboard' && (
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    {/* Quick Updates / Tips */}
                    <div className="bg-blue-600 rounded-[2.5rem] p-10 text-white relative overflow-hidden flex flex-col md:flex-row items-center gap-8 shadow-2xl shadow-blue-600/20">
                         <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-white/10 blur-[100px] rounded-full -mr-20 -mt-20" />
                         <div className="w-16 h-16 bg-white/10 rounded-2xl flex items-center justify-center flex-shrink-0">
                            <GraduationCap size={32} />
                         </div>
                         <div className="flex-1 text-center md:text-left">
                            <h3 className="text-xl font-black mb-2 uppercase italic">Mentor Tip of the Day</h3>
                            <p className="text-blue-100 font-medium">Providing a clear meeting link and setting expectation for sessions increases student satisfaction by 40%.</p>
                         </div>
                         <button 
                            onClick={() => setSearchParams({ tab: 'mentorship' })}
                            className="px-8 py-4 bg-white text-blue-600 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-gray-100 transition-all shadow-xl"
                         >
                            Check Requests
                         </button>
                    </div>
                    ...
                    <div className="bg-white border border-gray-100 rounded-[40px] p-10 shadow-sm">
                        ...
                        <div className="space-y-4">
                            {referrals.length > 0 ? referrals.map((ref) => (
                                <div 
                                    key={ref._id} 
                                    onClick={() => setSelectedJobForApplicants({ id: ref._id, title: ref.title })}
                                    className="group flex flex-col md:flex-row items-center justify-between p-6 bg-gray-50/50 hover:bg-white border border-transparent hover:border-gray-100 rounded-3xl transition-all cursor-pointer"
                                >
                                    ...
                                    <div className="flex items-center gap-4 mt-4 md:mt-0 w-full md:w-auto justify-between md:justify-end">
                                        ...
                                        <button className="px-5 py-2.5 bg-white border border-gray-200 rounded-xl text-[9px] font-black uppercase tracking-widest text-gray-600 group-hover:bg-blue-600 group-hover:text-white group-hover:border-blue-600 transition-all">
                                            View Applicants
                                        </button>
                                    </div>
                                </div>
