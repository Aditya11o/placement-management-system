      ...
      <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6 h-full hover:shadow-lg transition-all duration-300">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div className="col-span-2 md:col-span-1">
            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5 block pb-1">Preferred Locations</label>
            <input 
              type="text" 
              placeholder="e.g. Bangalore, Pune"
              value={Array.isArray(student.preferredLocations) ? student.preferredLocations.join(', ') : (student.preferredLocations || '')} 
              onChange={(e) => onChange('preferredLocations', e.target.value.split(',').map(s => s.trim()).filter(Boolean))}
              className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm font-medium focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all bg-gray-50/30" 
            />
          </div>
          <div className="col-span-2 md:col-span-1">
            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5 block pb-1">Preferred Roles</label>
            <input 
              type="text" 
              placeholder="e.g. SDE, Data Analyst"
              value={Array.isArray(student.preferredRoles) ? student.preferredRoles.join(', ') : (student.preferredRoles || '')} 
              onChange={(e) => onChange('preferredRoles', e.target.value.split(',').map(s => s.trim()).filter(Boolean))}
              className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm font-medium focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all bg-gray-50/30" 
            />
          </div>
        </div>
